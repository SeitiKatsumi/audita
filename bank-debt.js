...
function statementGuidance(){return `<details class="charge-no-documents">
 <summary>Não tenho os extratos completos. Como conseguir?</summary>
 <h3>Solicite os extratos ao seu banco</h3>
 <p>Peça o histórico completo desde o início do saldo devedor até a presente data, com todos os lançamentos e saldos. Se não souber quando a dívida começou, peça ajuda ao banco para localizar esse período.</p>
 <ol class="charge-no-documents-channels">
  <li><strong>1. Aplicativo do banco</strong><span>Abra a área da conta ou do cartão, procure Extrato ou Faturas, selecione o período e baixe ou compartilhe em PDF. Repita para os meses necessários. Os nomes das opções variam conforme o banco.</span></li>
  <li><strong>2. Internet banking</strong><span>Acesse o site oficial do seu banco e consulte os extratos ou faturas mês a mês. Exporte os documentos completos, incluindo todas as páginas.</span></li>
  <li><strong>3. Agência bancária</strong><span>Solicite ao gerente os meses que não aparecem nos canais digitais. Para uma conta encerrada, informe a agência e a conta e peça o histórico do período. No caixa eletrônico, confira se todo o período está disponível.</span></li>
  <li><strong>4. SAC do seu banco</strong><span>Use o contato disponível no aplicativo ou site oficial e solicite os documentos históricos. Anote o protocolo e o prazo de entrega informado. Se não resolver, procure a Ouvidoria do banco com esse protocolo.</span></li>
 </ol>
 <p><strong>Ao receber:</strong> confira o período, todas as páginas, os lançamentos e os saldos. Prefira o PDF original do banco; imagens precisam estar legíveis e sem cortes.</p>
 <p>Feche esta orientação e anexe os arquivos no campo abaixo. Abrir esta ajuda não apaga os arquivos já selecionados ou enviados.</p>
</details>`;}
function documentIntake(){
 ...
 const title=!current?'Entenda como sua dívida bancária chegou ao valor atual':'Envie seus extratos para análise';
 const explanation=!current?'<p>Sua dívida cresceu e você quer entender os juros e cobranças? Vou ajudar a conferir a composição do saldo devedor e identificar possíveis divergências nos documentos apresentados.</p><p>Para isso, preciso dos extratos desde o início da dívida até hoje. Eles permitem acompanhar as movimentações, os encargos e a evolução do saldo. Com informações suficientes, a análise ajuda a organizar os resultados em um relatório técnico e orientar os próximos passos. Ela não garante desconto ou acordo com o banco.</p>':'';
 const request=!current?'<h3>Envie seus extratos para análise</h3>':'<p>Anexe os extratos desde o início da dívida até hoje ou complemente os documentos já enviados.</p>';
 ...
 return bubble((intakeNotice?'<p role="alert">'+esc(intakeNotice)+'</p>':'')+'<h2>'+title+'</h2>'+explanation+request+statementGuidance()+(current?.analysisError?'<p role="alert">Não foi possível concluir a leitura. Seus arquivos estão salvos. Tente analisar novamente.</p>':'')+(hasFiles?'<p class="debt-note">Os arquivos enviados já estão salvos. Não é necessário anexá-los outra vez.</p>':'')+(current&&!current.owner?'':form('documents',fileInput('files',current?.status==='offer'||!hasFiles)+analysisConsent(),label))+documentList()+analysisResult()+ (current?reviewer():''));
}
