...
export async function analyzeStatements(documents,{rateProvider=fetchDebtRate}={}){
 ...
 for(let i=1;i<ordered.length;i++){
  const previous=ordered[i-1],file=ordered[i],a=previous.data,b=file.data;
  // Only carry an explicitly printed closing balance across the same dated boundary.
  if(!a.accountKey||a.accountKey!==b.accountKey||a.bank!==b.bank||a.person!==b.person||a.modality!==b.modality||
    !a.closing||!b.opening||!b.closing||b.opening.balanceCents!==0||a.closing.balanceCents===0||
    a.closing.date!==b.opening.date||!b.entries.length||!a.entries.every(r=>r.date<=a.closing.date)||!b.entries.every(r=>r.date>a.closing.date&&r.date<=b.closing.date))continue;
  let balance=a.closing.balanceCents,consistent=true,anchors=0;
  for(const row of b.entries){balance+=row.amountCents;if(row.balanceCents!==null){anchors++;if(Math.abs(balance-row.balanceCents)>2)consistent=false;}}
  if(!anchors||!consistent||Math.abs(balance-b.closing.balanceCents)>2)continue;
  const resolved=new Set((b.checkpoints||[]).map(c=>`Página ${c.page}, linha ${c.line}: os movimentos não fecham com o saldo impresso.`));
  reconciliations.push({documentId:file.id,previousDocumentId:previous.id,originalOpening:b.opening,openingBalanceCents:a.closing.balanceCents,date:a.closing.date});
  b.opening={...a.closing};b.issues=b.issues.filter(message=>!resolved.has(message));
 }
 ...
 if(rows.some(r=>r.kind==='late_fee'))add('Conferir a composição da mora: base de cálculo, taxas e períodos de cada encargo. O total isolado não permite recalculá-la.');
 ...
 if(opening&&closing){
  ...
  let balance=opening.balanceCents,mismatch=false;
  for(const row of rows){balance+=row.amountCents;if(!mismatch&&row.balanceCents!==null&&Math.abs(balance-row.balanceCents)>2){
   const difference=Math.abs(balance-row.balanceCents);issues.push({documentId:row.documentId,differenceCents:difference,message:`Conferir saldo anterior e movimentações até ${row.date}: diferença de ${new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'}).format(difference/100)} frente ao saldo impresso. Essa diferença não foi presumida como dívida ou juros.`});mismatch=true;
  }}
  if(!mismatch&&Math.abs(balance-closing.balanceCents)>2)add('A soma das movimentações não corresponde ao saldo final.');
