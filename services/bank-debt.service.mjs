...
export function createBankDebtService({getDb,checkout,now=()=>new Date()}) {
  ...
  async function command(auth,id,input,requestInfo={}){return tx(async c=>{
    ...
    if(action==='answer'){
      ...
      p.details=debtParse(debtDetails,input.details);p.review=null;r.status='calculation_pending';
    }else if(action==='review'){
      debtRequire(operator(auth),'Acesso restrito à equipe responsável.',403);debtRequire(['calculation_pending','offer'].includes(r.status),'O caso não está disponível para revisão.',409);
      const review=debtParse(debtReview,input.review);debtRequire(review.reviewedCents<p.details.chargedCents,'O cálculo precisa indicar uma diferença positiva para esta contratação.');
      ...
      p.review={...review,id:randomUUID(),by:auth.user.id,at:now().toISOString(),version:DEBT_VERSION};r.status='offer';
  ...
  async function upload(auth,id,{bytes,name,kind}){return tx(async c=>{
    const r=await access(c,auth,id,true);owner(r,auth);debtRequire(['calculation_pending','paid','signature'].includes(r.status),'Anexos não podem ser alterados nesta etapa.',409);
    debtRequire(['identity','address','evidence'].includes(kind),'Tipo de documento inválido.');
    if(r.status==='calculation_pending')debtRequire(kind==='evidence','Envie agora os documentos da dívida.');
    ...
    await save(c,r,auth,'document_uploaded');return view(c,r,auth);
  });}
  async function createCheckout(auth,id,input){return tx(async c=>{
    ...
    debtRequire(['offer','payment_pending'].includes(r.status)&&p.review,'O cálculo Audita precisa ser validado antes da contratação.',409);
    debtRequire(input.accepted===true&&input.reviewId===p.review.id,'Confirme a análise e as condições atuais.');
    ...
    const session=await checkout(auth,{caseId:id,reviewId:p.review.id,amountCents:p.review.priceCents,attempt:p.attempt});
    debtRequire(session.id&&/^https:\/\//.test(session.url),'Não foi possível abrir o pagamento.',503);
    p.checkout=session;p.purchaseAcceptance={version:DEBT_VERSION,at:now().toISOString(),reviewId:p.review.id,terms:DEBT_TERMS};r.status='payment_pending';await save(c,r,auth,'checkout_created');return session;
  });}
  async function paymentEvent(event){
    ...
    return tx(async c=>{
      ...
      debtRequire(String(m.audita_user_id)===String(r.user_id)&&String(m.audita_tenant_id)===String(r.tenant_id)&&m.debt_review_id===p.review?.id&&o.id===p.checkout?.id,'Pagamento não corresponde ao atendimento.',409);
      if(!p.paid){
        if(['checkout.session.expired','checkout.session.async_payment_failed'].includes(event.type)){r.status='offer';p.checkout=null;p.paymentProcessing=false;}
          ...
          debtRequire(o.currency==='brl'&&o.amount_total===p.review.priceCents,'Valor do pagamento divergente.',409);
