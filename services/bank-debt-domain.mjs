...
export const DEBT_QUESTIONS=[
  ...
  {key:'notified',title:'Você está recebendo notificações de cobrança dessa dívida?',yes:'Sim, estou recebendo cobranças',no:'Ainda não recebi cobranças'},
  {key:'excessive',title:'Você considera que os juros cobrados estão muito acima do valor que ficou em aberto?',yes:'Sim, o valor aumentou muito',no:'Não / não tenho certeza'},
];
...
export const debtDetails=z.object({creditor:text,kind:z.enum(['overdraft','card','loan','financing','other']),since:date,originalCents:cents.refine(v=>v>0),chargedCents:cents.refine(v=>v>0),offeredCents:cents,description:z.string().trim().max(3000),consent:z.literal(true)}).strict();
