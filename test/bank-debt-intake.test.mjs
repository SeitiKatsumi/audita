...
test('envio único, leitura sem anexos e complemento sem repetir autorização',async()=>{
 ...
 assert.match(html(),/Seus extratos estão em análise/);assert.doesNotMatch(html(),/type="file"|name="consent"|Anexar documento/);assert.equal((html().match(/<li>/g)||[]).length,1);
 ...
 assert.match(html(),/Analisar extratos enviados/);assert.doesNotMatch(html(),/name="files" multiple required/);
 ...
 assert.match(html(),/Plano de teste/);assert.match(html(),/Contratar e continuar/);assert.doesNotMatch(html(),/type="file"|Enviar complemento/);
});
